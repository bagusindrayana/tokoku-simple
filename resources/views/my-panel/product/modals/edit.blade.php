@push('scripts')
    <script>
        ...
        //on edit-data button click
        $(document).on('click', '.edit-data', function() {
            //show modal
            window
                .FlowbiteInstances
                .getInstance('Modal', 'updateProductModal').show();
            // updateProductModal.removeClass('hidden');
            // updateProductModal.addClass('flex');
            updateProductModal.find(".relative .loading").show();
            //get id
            var id = $(this).data('id');
            //get data
            $.ajax({
                ...
                success: function(response) {
                    //fill form with data
                    $('#updateProductModal').find('input[name="product_name"]').val(response.data
                        .product_name);
                    $('#updateProductModal').find('input[name="product_price"]').val(response.data
                        .product_price);
                    $('#updateProductModal').find('textarea[name="product_description"]').val(response
                        .data.product_description);
                    //set form action
                    $('#updateProductModal').find('form').attr('action',
                        '{{ route('api.my-panel.product.update', 0) }}'
                        .replace('0', id));
                ...
                complete: function() {
                    //hide loading
                    updateProductModal.find(".relative .loading").hide();
